...
Mailin.prototype.start = function (options, callback) {
    ...
    function validateAddress(addressType, email, session) {
        ...
        return (new Promise(function (resolve, reject) {
            ...
            if (_this.configuration.blockSpfFailed && addressType === 'recipient') {
                return mailUtilities.validateSpfAsync(session.remoteAddress, session.envelope.mailFrom.address, session.clientHostname)
                    .catch(function (err) {
                        logger.error(session.id + ' Unable to validate spf. Consider spf as failed.');
                        logger.error(err);
                        throw err;
                    }).then(function () {
                        return;
    ...
    function validateSpf(connection) {
        ...
        /* Get ip and host. */
        return mailUtilities.validateSpfAsync(connection.remoteAddress, connection.envelope.mailFrom.address, connection.clientHostname)
            .catch(function (err) {
    ...
    function onData(stream, session, callback) {
        ...
        stream.on('end', function () {
            if (stream.sizeExceeded) {
                const err = new Error('Message size exceeds ' + configuration.smtpOptions.size + 'KB');
                err.responseCode = 552;
                // 有错误情况下清理文件
                unlinkFile(connection).catch(function (err) {
                    logger.error(err);
                });
                return callback(err);
            }
    ...
    function onMailFrom(address, session, streamCallback) {
        _this.emit('validateSenderNew', session, address.address, streamCallback);
    ...
    function onRcptTo(address, session, streamCallback) {
        ...
        if (expectedSize > configuration.smtpOptions.size) {
        ...
        _this.emit('validateRecipientNew', session, address.address, streamCallback);
